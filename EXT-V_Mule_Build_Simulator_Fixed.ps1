# Cadillac EXT-V Mule Build Simulator - Basic Version
...
for ($week = $StartWeek; $week -le $EndWeek; $week++) {
    ...
    elseif ($week -eq 16) {
        ...
        if ($deliverySuccess) {
            ...
            $supplierDelays++
            # Simulate a chance for a simple delay to escalate into a critical issue
            $escalates = (Get-Random -Minimum 1 -Maximum 100) -le 15 # 15% chance
            if ($escalates) {
                Write-Host "→ The supplier delay has caused a knock-on effect, escalating to a CRITICAL ISSUE." -ForegroundColor Red
                $criticalIssues++
            }
        }
    }
